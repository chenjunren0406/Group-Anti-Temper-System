package RMIPack.rmiUtils;
import GASS.utils.GetIP;
import RMIPack.rmiUtils.classServer.ClassFileServer;
...
public class RMIUtils implements IRMIUtils {
	 ...
	 */
	@Override
	public Registry getLocalRegistry(int port) {
	 ...
	 */
	@Override
	public Registry getRemoteRegistry(String host, int port) {
	 ...
	 */
	@Override
	public String startRMI(int classServerPort, boolean isServer){
		configSecurityManager();
		String ret = configRMIProperties(classServerPort, isServer);
	 ...
	 */
	private String configRMIProperties(int classServerPort, boolean isServer) {
		// Logs all RMI activity to System.err
		System.setProperty("java.rmi.server.logCalls", "true"); 
		try {
			// Try to get figure out this server's IP address and save it as the

			/* force java to use ipv4 -- yanfei */
			System.setProperty("java.net.preferIPv4Stack", "true");
			// RMI server hostname.
			//String retIP = isServer ? GetIP.GetPublicIP() : GetIP.getClientPublicIP();
			String retIP = getLocalAddress();
			System.setProperty("java.rmi.server.hostname", retIP);
	...
	private void configSecurityManager() {
		...
		System.setProperty("java.security.policy", "RMIPack" + sep
